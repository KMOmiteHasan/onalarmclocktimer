  ...
  function a() {
    ...
    if (!isEmbed) {
      document.cookie =
        ...
        "; path=/; max-age=63072000; secure; samesite=strict";
    }
    $("#color-style").html(
      ".colored {color:" +
...
var audioC = (function () {
  ...
  if (audioEdit.length) {
    try {
      ...
      audioSupported = !!(
        o.canPlayType && o.canPlayType("audio/mpeg;").replace(/no/, "")
      );
    } catch (m) { }
  }
  function c(a) {
  ...
  function k(a) {
    ...
    reader.onload = function (u) {
      try {
        if (
          databaseC !== undefined &&
          databaseC.objectStoreNames.contains(dbTableName)
        ) {
          databaseC
            .transaction(dbTableName, "readwrite")
            .objectStore(dbTableName)
            .put(reader.result, dbTableId);
        } else {
  ...
  function d(a) {
    ...
    if (audioIndex !== langC.audioNames.length) {
      ...
      if (
        ...
        transaction.objectStore(dbTableName).get(dbTableId).onsuccess =
          function (e) {
    ...
    configC[audioTool + "AudioCode"] =
      audioIndex !== langC.audioNames.length
        ? langC.audioNames[audioIndex][1]
        : "custom";
  ...
  function n() {
    return (
      ...
      configC[audioTool + "AudioCode"] +
      "&loop=" +
      (configC[audioTool + "AudioLoop"] ? "1" : "0")
    );
  }
  function b() {
    if (isEmbed) {
  ...
  return { init: c, set: j, play: g, pause: f, reload: p, url: n, check: b };
})();
...
if (window.innerWidth <= 768) {
  console.log("Ikuuuuu...")
  document.body.classList.remove("open-right-sidebar")
} else {
  console.log("Nope...")
  document.body.classList.add("open-right-sidebar")
}